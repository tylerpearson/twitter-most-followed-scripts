...
accounts_to_check.each_with_index do |account_username, account_index|
  ...
  begin
    ...
    accounts_friends.take(5000).each_with_index do |friend, account_friends_index|
      ...
      if accounts_to_check.include?(friend)
        puts "#{account_username} is following #{friend}".green
        matched << [account_username, friend]
      else
        puts "#{account_username} is following #{friend}".red
      end

    end

    File.open("relationships/#{GROUP}.json","w") do |f|
    ...
    if num_attempts <= MAX_ATTEMPTS
      puts "Rate limit error, sleeping for #{error.rate_limit.reset_in / 60} minutes...".red
    ...
    puts "HEY THIS IS A RANDOM ERROR".blue
